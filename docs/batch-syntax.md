...
批量语法用于脚本工作区。Web 界面使用 `form` 模式，CLI 的 `add` 和 `run --config` 也会展开批量配置。
...
展开会保留原始键和字典结构：字面键 `"a.b"` 与嵌套的 `a: {b: ...}` 是两个独立字段，数字键和空字典也会保留。
模板类型校验和配置内容搜索也会分别处理这些字段。
列表整体作为固定值，其中的字符串不参与批量展开。
