...
任务管理器通过 `read_task_payload_snapshot` 同时取得载荷和已打开文件的签名，减少加载阶段重复的路径解析。签名在读取字节前采集，读取或解析失败时也保留已取得的签名；后续刷新继续检查文件代次与属性变化，避免把旧内容标成新文件，或反复解析未变化的错误配置。`read_task_payload` 保留原四元组返回值。
