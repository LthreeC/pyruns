...
![Generator 页面概览](assets/配置参数页面.png)
...
![Manager 页面概览](assets/任务卡片页面1.png)
...
- **勾选框**：勾选任务用于导出
- **监控徽章**：显示 `add_monitor()` 写入的数据条数（每个运行包含 1 条最终指标记录）
- **手动刷新**：点击 ↻ 按钮手动同步磁盘
...
导出的数据来源于 `pyruns.add_monitor()` 写入的最终指标条目。
