...
![Generator 页面概览](assets/ui_generator_refined.png)

### 界面风格与性能

- 当前界面风格已经统一为更克制的浅色工作台风格：更小的圆角、更轻的阴影、更稳定的间距节奏。
- `Generator / Manager / Monitor` 共用同一套表面样式规则，避免每个页面都像不同项目。
- 未访问的标签页不再预渲染，页面切换时的初始开销更低。
- `Manager` 和 `Monitor` 的刷新策略更偏向“只在必要时更新”，长时间运行时的无效重绘更少。
