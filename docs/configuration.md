...
正则使用 Python `regex` 语法并逐行匹配，例如 `loss=\d+\.\d+`，支持分组、或条件及前后查找。全词匹配要求两侧不是 Unicode 单词字符（字母、数字、下划线等）。正则模式保留原始空格；普通文本仍兼容配置中冒号两侧的空格差异。

格式错误或耗时过长的表达式会显示错误。启用全词或正则匹配时，单条日志行超过 8 Mi 字符也会明确报错；这不是整个日志文件的大小限制，关闭全词和正则匹配后仍可使用分块的普通文本搜索（包括区分大小写）。

搜索结果仅展示有限数量的预览，数量上限不会截断检索范围。大日志首次搜索需要扫描文件；再次搜索未变化的日志可使用缓存。输入有防抖，旧请求可以取消；搜索期间不会自动反复扫描全量日志，需要查看新写入的匹配时点击刷新。
