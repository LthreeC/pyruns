...
令牌会换成持久的 `HttpOnly` 会话 cookie，并从地址栏移除。该会话会在使用时续期，
同一主机、端口和工作区的正常更新或重启不会令其过期。清除浏览器数据或本地
`~/.pyruns/sessions` 状态后，需要重新打开终端打印的最新完整 URL。不要分享该 URL；
这个机制用于隔离同机其他进程，不是远程多用户部署的身份系统。
