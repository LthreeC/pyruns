...
class TaskGenerator:
    ...
    def create_task(
        ...
        display_name = name_prefix if name_prefix else folder_name
        if group_index:
            display_name += f" [{group_index}]"

        task_obj = create_task_object(task_id, task_dir, display_name, config)

        # ── 写入文件 ──
        # task_info.json: 基本元数据
        info = {k: v for k, v in task_obj.items() if k not in ["dir", "config", "log"]}
        # 从环境变量获取 script 路径 (pyr 模式)
        ...
        # config.yaml: 原样保存，不做任何过滤
        save_yaml(os.path.join(task_dir, CONFIG_FILENAME), config)
    ...
    def create_tasks(
        ...
        for idx, cfg in enumerate(configs):
            ...
            tasks.append(self.create_task(name_prefix, cfg, use_timestamp_name, group_index))
        return tasks
