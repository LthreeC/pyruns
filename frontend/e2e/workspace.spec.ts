...
test('update wait exits with a retryable error when the backend never restarts', async ({ page }) => {
  ...
  let resolveRestartPoll!: () => void
  const restartPoll = new Promise<void>(resolve => {
    resolveRestartPoll = resolve
  })
  await page.route('**/api/system/info', route => {
    if (updateStarted) resolveRestartPoll()
    return route.fulfill({
      json: {
        version: '0.3.3',
  ...
  await restartPoll
  await page.clock.fastForward(480_500)
...
for (const workspaceKind of ['script', 'shell'] as const) {
  test(`Monitor and Manager combine search fields and match options in ${workspaceKind} workspaces`, async ({ page, isMobile }, testInfo) => {
    if (isMobile) await page.setViewportSize({ width: 375, height: 667 })
    const errors: string[] = []
    page.on('pageerror', error => errors.push(error.message))
    const contentField = workspaceKind === 'shell' ? 'script' : 'config'
    const fields = ['name', 'notes', contentField, 'env', 'log']
    let lastSearch = new URLSearchParams()
    const task = { name: 'needle-demo', dir: '/tmp/needle-demo', status: 'completed', task_kind: workspaceKind === 'shell' ? 'shell' : 'config', run_index: 1, notes: '' }
    await page.route('**/api/workspace', async route => {
      const response = await route.fetch()
      await route.fulfill({ response, json: { ...await response.json(), workspace_kind: workspaceKind } })
    })
    await page.route('**/api/tasks?*', route => {
    ...
    for (const view of ['Monitor', 'Manager']) {
      ...
      const filters = page.getByRole('toolbar', { name: 'Search filters', exact: true })
      const searchControl = filters.locator('..')
      const filterTrigger = page.getByRole('button', { name: 'Search filters', exact: true })
      await expect(search).toBeVisible()
      const compactFilters = await filterTrigger.isVisible()
      if (compactFilters) {
        await filterTrigger.click()
        await expect(filters).toBeVisible()
        await expect(filters).toBeInViewport({ ratio: 1 })
        await page.screenshot({ path: testInfo.outputPath(`${view}-search-filters-open.png`), animations: 'disabled' })
      }
      await expect(selector).toHaveValue('all')
      const filterBounds = await filters.boundingBox()
      const controlBounds = await searchControl.boundingBox()
      ...
      const regexBounds = await page.getByRole('button', { name: 'Use regular expression', exact: true }).boundingBox()
      ...
      if (compactFilters) {
        ...
        expect(Math.abs(matchBounds!.y - searchBounds!.y)).toBeLessThanOrEqual(1)
        expect(Math.abs(matchBounds!.y - selectorBounds!.y)).toBeLessThanOrEqual(1)
        expect(matchBounds!.x - (selectorBounds!.x + selectorBounds!.width)).toBeLessThanOrEqual(2)
        expect(filterBounds!.width).toBeLessThanOrEqual(260)
        expect(filterBounds!.x).toBeGreaterThan(searchBounds!.x)
        expect(filterBounds!.y + filterBounds!.height).toBeLessThanOrEqual(searchBounds!.y + searchBounds!.height + 1)
        expect(regexBounds!.x + regexBounds!.width).toBeLessThanOrEqual(controlBounds!.x + controlBounds!.width + 1)
      }
      ...
      for (const field of [...fields, 'all']) {
        ...
        await expect(group.locator('mark')).toHaveCount(field === 'all' ? fields.length : 1)
        await expect.poll(() => lastSearch.get('search_field')).toBe(field)
...
test('task detail only offers replacement after loading the newer value', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Save Notes' })).toBeVisible()
  await expect(page.getByText('Could not load newer notes', { exact: true })).toHaveCount(0)
  ...
  await expect(page.getByText('Could not load newer environment', { exact: true })).toHaveCount(0)
